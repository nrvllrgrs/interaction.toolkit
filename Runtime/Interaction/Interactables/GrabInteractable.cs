...
namespace UnityEngine.Interaction.Toolkit
{
    ...
    [AddComponentMenu("Interaction/Interactables/Grab Interactable")]
    public class GrabInteractable : BaseInteractable
    {
        ...
        [SerializeField]
        BaseInteractable.MovementType m_MovementType = MovementType.Kinematic;
        /// <summary>Gets or sets movement type for RigidBody.</summary>
        public BaseInteractable.MovementType movementType { get { return m_MovementType; } set { m_MovementType = value; } }
        ...
        // Interactor
        BaseInteractor m_SelectingInteractor;
        /// <summary>Get the current selecting interactor for this interactable.
        public BaseInteractor selectingInteractor { get { return m_SelectingInteractor; } }
        ...
        BaseInteractable.MovementType m_CurrentMovementType;
        ...
        public override void ProcessInteractable(InteractionUpdateOrder.UpdatePhase updatePhase)
        {
            switch (updatePhase)
            {
                //
                ...
                //
                case InteractionUpdateOrder.UpdatePhase.Fixed:
                    {
                        if (isSelected)
                        {
                ...
                // the call to MoveToTarget will perform the correct the type of update depending on the update phase
                //
                case InteractionUpdateOrder.UpdatePhase.Dynamic:
                    {
                        if (isSelected)
                        {
                ...
                // the call to MoveToTarget will perform the correct the type of update depending on the update phase
                //
                case InteractionUpdateOrder.UpdatePhase.OnBeforeRender:
                    {
                        if (isSelected)
                        {
                            UpdateTarget(Time.unscaledDeltaTime);
                ...
                //
                case InteractionUpdateOrder.UpdatePhase.Late:
                    {
        ...
        Vector3 GetWorldAttachPosition(BaseInteractor interactor)
        {
            return interactor.attachTransform.position + interactor.attachTransform.rotation * m_InteractorLocalPosition;
        }
        Quaternion GetWorldAttachRotation(BaseInteractor interactor)
        {
        ...
        void PerformInstantaneousUpdate(float timeDelta, InteractionUpdateOrder.UpdatePhase updatePhase)
        {
            if (updatePhase == InteractionUpdateOrder.UpdatePhase.Dynamic ||
                updatePhase == InteractionUpdateOrder.UpdatePhase.OnBeforeRender)
            {
                if (trackPosition)
                {
        ...
        void PerformKinematicUpdate(float timeDelta, InteractionUpdateOrder.UpdatePhase updatePhase)
        {
            if (updatePhase == InteractionUpdateOrder.UpdatePhase.Fixed)
            {
                if (trackPosition)
                {
        ...
        void PerformVelocityTrackingUpdate(float timeDelta, InteractionUpdateOrder.UpdatePhase updatePhase)
        {
        ...
        void UpdateInteractorLocalPose(BaseInteractor interactor)
        {
		...
		protected internal override void OnSelectEnter(BaseInteractor interactor)
		{
            if (!interactor)
                return;
            ...
            m_RigidBody.isKinematic = (m_CurrentMovementType == BaseInteractable.MovementType.Kinematic);
		...
		protected internal override void OnSelectExit(BaseInteractor interactor)
		{
        ...
        public override bool IsHoverableBy(BaseInteractor interactor) 
        { 
            return true;
        }
